...
def main(mode, project_directory = ''):
    ...
    section_config['counts_norm'] = ainput('Population size to normalize counts by (-1 if normalization is not required): ', -1, existing_cfg[section_name]['counts_norm']).getInteger()
    ...
    input_files_ratios = [[config['sel_filename_aligner'] + '_PRO_qc',config['input_filename_aligner'] + '_PRO_qc'],[config['sel_filename_aligner'] + '_PRO_qc.m1',config['input_filename_aligner'] + '_PRO_qc.m1'],[config['sel_filename_aligner'] + '_PRO_qc.m2',config['input_filename_aligner'] + '_PRO_qc.m2']]
    freqplot_list = []
    ratioplot_list = []
	...
	for ind,l in enumerate(input_files_ratios, start=len(ratioplot_list)):
	    ratioplot_list.append([])
	    for filename in l:
	        ratioplot_list[ind].append('unlink_' + item + '_' + filename)
		
    section_config['input_filename_freqplot'] = ','.join(freqplot_list)
    section_config['input_filename_ratioplot'] = ';'.join( [ ','.join(l) for l in ratioplot_list ] ) 
